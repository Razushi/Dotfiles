...
PASSWD_CLEAN="$(echo -n "$PASSWD" | jq -Rs . | sed -e 's/\"//g')"

CURL_RESULT=$(curl -sSL -X POST 'https://account.hypr.land/api/v1/login' --data "{\"username\":\"$UNAME\", \"password\":\"$PASSWD_CLEAN\", \"totp\": \"$TOTP\", \"desktop\":true}")
