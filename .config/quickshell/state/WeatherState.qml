...
Singleton {
  ...
  property string weatherStr: "Unknown"
  ...
  Process {
    id: weatherProc
    running: false
    command: ["bash", "-c", "curl -s \"wttr.in/" + (isWeatherOverridden ? C.Config.settings.bar.weatherLocation : "$(curl -s ipinfo.io/city)") + "?format=%c|%t|%C|%f|%m|%p|%l&lang=$(tr -dc 'a-z' </dev/urandom | head -c32)\""]
